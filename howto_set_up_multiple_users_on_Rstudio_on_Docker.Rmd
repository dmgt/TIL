...
- [Jetstream](https://jetstream-cloud.org/) is an National Science Foundation supported cloud compute resource that runs on the open-source software Atmosphere and OpenStack. Compute credit allocations are avalible by applying through XCSEDE. If you can contact campus research computing staff, they may be able to provide advice and example applications (eg at UC Berkeley this is the [Research Computing](https://research-it.berkeley.edu/programs/berkeley-research-computing) group)  

-[Digital Ocean](https://www.digitalocean.com/) is a commercial cloud compute resource (similar to AWS or Azure) offering similar instances to those avalible on Jetstream. Compute credits are billed hourly, depending on the size of the resource. 
...
- Once the droplet is set up, connect to the droplet's shell interface by opening a terminal and connecting vis ssh with 
```
