Instructions for setting up RStudio running in a Docker container on
...
Dana Miller
  ...
  - [Set up a new instance for the first
    time](#set-up-a-new-instance-for-the-first-time)
      - [On Jetstream](#on-jetstream)
      - [On Digital Ocean](#on-digital-ocean)
  - [Start a Docker container with R and
    RStudio](#start-a-docker-container-with-r-and-rstudio)
  - [Configure git with `git config`](#configure-git-with-git-config)
  - [Optional next step - adding multiple users and configuring their
    permissions](#optional-next-step---adding-multiple-users-and-configuring-their-permissions)
      - [1. Add more users to an existing
        instance](#add-more-users-to-an-existing-instance)
      - [2. Launch a separate R Studio instance bound to a different
        port](#launch-a-separate-r-studio-instance-bound-to-a-different-port)
  - [Optional next steps - installing `curl`, `pip3` and `aws cli` in
    the terminal after RStudio is
...
for cloud compute resources through
[Jetstream](https://jetstream-cloud.org/),
[DIBSCI](http://ivory.idyll.org/dibsi/2018/WHO.html) at UC Davis for
documentation, and Carl Boettiger at UC Berkeley for reproducible
research inspiration and an initial version of these instructions*
...
both of these platforms. Most of the instructions below apply to both
platforms, anything that is Jetstream or Digital Ocean-specific will be
noted.
...
If you already have a Jetstream login and allocation, to run R Studio on
Jetstream:

  - Log in and go to ‘Create a new project’
  - Enter project name and description
  - Select the size and *image* you would like to use (images have
    various software preinstalled): select
    [this](https://use.jetstream-cloud.org/application/images/107)
    Ubuntu image that includes Docker and GUI support
  - Once the new instance for your project is active (green dot), click
    the “Open web shell” link on the right hand side

#### On Digital Ocean

  - After creating a Digital Ocean account, create a new instance using
    [this](https://marketplace.digitalocean.com/apps/docker) Ubuntu +
...
### Start a Docker container with R and RStudio
  ...
  - In the web shell, first confirm you are able to run docker commands:
    ...
    If you get an error about permission denied and docker daemon, try
    adding yourself to root privileges with `sudo usermod -aG docker
    example_username`, where `example_username` is your username
    
      - Start a new Docker container that has R, R Studio, and the
        `tidyverse` preinstalled (you could also install these and all
        the required dependencies without Docker but it takes a long
        time and is potentially error-prone)
...
Note the URL above is *http* not *https*, the latter is also possible to
set up but requires a custom domain name

Note to copy and paste text from the Jetstream web shell, you have you
highlight it and then press Ctrl + Alt + Shift

  - Paste the URL above into your web browser, and to login:
    
      - The default username is “rstudio”
      - The password is whatever you chose in the `docker run...`
        command above for `example_password`
...
Optional UNIX configuration notes:
  ...
  - `whoami` in at the command line is *not* the same as the user for
    logging in to the R Studio browser prompt
  - Specifying a username with `docker run -u...` also doesn’t work for
    the R Studio login prompt

### Configure git with `git config`

If you will be using `git` to incrementally save your work on the
...
#### 1\. Add more users to an existing instance
  ...
  - See the ids of your running Docker container with RStudio by running
    `docker ps`. You will need to write down or remember the first 5
    digits of your CONTAINER ID (a long string of numbers and letters)
...
Where CONTAINER\_ID is eg “904b2”, the first 5 digits. Notice your
prompt will change now that you are inside the container.

  - Now you can use regular UNIX commands to create a new user (this is
    not Docker-specific), where `example_user` is the user name you are
    creating (the name can be anything you choose).
...
Enter a password, press ‘Enter’ to leave the other questions blank (eg
Name, Work Phone…), they are not required, and press Y for “Yes”

  - At this point, someone could go to the same URL you set up above,
    and login with this new username and password *but* they wouldn’t be
    able to install any additional R packages (no root access)

  - To give the new user the right to download packages, add them to the
    `staff` group. In the command below, `-aG` means ‘append to group’
...
To see a list of all the users (this includes some other system info) as
well
...
Alternatively, start an additional Docker container on the same
Jetstream instance with a different port number

``` bash
docker run -d -p 8888:8787 -rm -e PASSWORD=newpassword rocker/tidyverse 
```

The default username will still be “rstudio”, and the password will be
whatever you just specified, but the login URL will be different (note
the port number is changed above and accordingly also changed below)
...
### Optional next steps - installing `curl`, `pip3` and `aws cli` in the terminal after RStudio is running
  ...
  - [`curl`](https://curl.haxx.se/) is a common command line utility
    that you will probably need if you are downloading data or files
    from URLs
  - `pip3` is the package manager for the `python` programming language.
    In this case, it’s required to install `aws cli`
...
1.  Install curl, eg following instructions
    [here](https://www.luminanetworks.com/docs-lsc-610/Topics/SDN_Controller_Software_Installation_Guide/Appendix/Installing_cURL_for_Ubuntu_1.html)
...
<!-- end list -->
  ...
  - See “Install pip” instructions
    [here](https://docs.aws.amazon.com/cli/latest/userguide/install-linux.html#install-linux-pip)
      - `verse` image has python three, so use the python3 variants, eg
        `python3 get-pip.py --user`, not `python get-pip.py --user`
    
      - For the step about editing your bash profile, it will probably
        be called `.profile`.
    
      - You can use `vim` to edit the file (type `vim .profile` to open
        the file:
        
          - `i` to go into “insert” (editing) mode
          - add `export PATH=~/.local/bin:$PATH` to the file below
            anything in it already (you can start a line with `#` as a
            comment to explain why it’s there)
          - press `:` to exit editing mode and `wq` to save (write) and
            quit to go back to the terminal
    
      - Per the AWS docs, make sure you source the file after editing
...
[here](https://docs.aws.amazon.com/cli/latest/userguide/install-linux.html#install-linux-awscli)
...
### More references
  ...
  - DIBSCI docs on [setting up a Jetstream
    ...
    Jetstream](https://angus.readthedocs.io/en/2018/visualizing-blast-scores-with-RStudio.html#installing-and-running-rstudio-on-jetstream)
    - note some of the instructions are specific to their workshop
  - See more details from the Rocker Project on managing user
    authentication
    [here](https://www.rocker-project.org/use/managing_users/)
